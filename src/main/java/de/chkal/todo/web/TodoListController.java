...
public class TodoListController {
   ...
   /**
    * Executed when the user navigates to the page. The method uses MVC's Models class to populates the model with all
   ...
   /**
    * Handles data submitted by the "create item" form. The form data is validated using Bean Validation annotations.
    * The method uses MVC's ValidationResult class to access the detected violations. If violations were found, they are
    * added to the custom Messages class which is used by the view for rendering.
    */
   @POST
   @Path("/create")
   ...
   /**
    * Handles deletion of items. Works basically the same as the createItem() method. The method doesn't use a separate
    * from class, because there is only a single hidden field representing the id of the item.
    */
   @POST
   @Path("/delete")
